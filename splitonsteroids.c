...
/*   Updated: 2023/11/04 15:18:47 by aguediri         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int ft_wordcount(char const *s, char c)
{
    ...
    while (s[i])
    {
        ...
        if (((s[i] != c && s[i] != ' ' && s[i] != '\t') || inside_single_quotes || inside_double_quotes) &&
            (i == 0 || (s[i - 1] == c || s[i - 1] == ' ' || s[i - 1] == '\t')))
        {
            count++;
        }
        i++;
...
static int ft_split_len(const char *s, int l, char c)
{
    ...
    while ((s[l + i] != c && s[l + i] && s[l + i] != ' ' && s[l + i] != '\t') || inside_single_quotes || inside_double_quotes)
    {
        if (s[l + i] == '\'')
            inside_single_quotes = !inside_single_quotes;
        else if (s[l + i] == '"')
...
static int ft_split_start(const char *s, char c, int l)
{
    ...
    while ((s[l] == c || s[l] == ' ' || s[l] == '\t') && !(inside_single_quotes || inside_double_quotes))
    {
        if (s[l] == '\'')
            inside_single_quotes = !inside_single_quotes;
        else if (s[l] == '"')
